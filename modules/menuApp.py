...
def muestra_barra_lateral():
    ...
    # Mostrar recordatorio de la BD activa en páginas clave
    if st.session_state.paginaActual in ['calculo_grupos', 'analisis']:
        ...
        if 'bdActual' in st.session_state and st.session_state['bdActual']:
            ...
            st.sidebar.success(f"🗄️ **BDD activa:**\n\n`{nombreBd}`")
        else:
            st.sidebar.warning("⚠️ **No hay ninguna base de datos activa.**")



def muestra_pantalla_principal():
    st.markdown("<h1 style='text-align: center'>Bienvenido a la aplicación de planificación docente de la FCT</h1>", unsafe_allow_html = True)
    
    if 'bdActual' in st.session_state and st.session_state['bdActual']:
        nombreBd = os.path.basename(st.session_state['bdActual'])
        st.success(f"🗄️ **Base de datos activa:** `{nombreBd}`")
    else:
        st.warning("⚠️ **No hay ninguna base de datos activa.** Ve a **Configuración de la base de datos** para seleccionar o crear una.")
