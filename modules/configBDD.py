...
# 🔥 IMPORTACIÓN DEL "MODO DIOS" PARA REPARTO AUTOMÁTICO
try:
    ...
    try:
        ...
        recalcula_distribucion_total = None
...
def crea_bdd_excel(archivoSubido, nombreBd):
    try:
        ...
        # 🔥 MODO DIOS: Ejecutamos el recálculo y distribución física automáticamente 🔥
        if recalcula_distribucion_total and gruposCols:
...
def muestra_pagina_config():
    ...
    archivosBd = glob.glob(os.path.join(directorioBase, "*.db"))# + glob.glob(os.path.join(directorioBase, "*.sqlite"))
    ...
    if len(archivosBd) > 0:
        ...
        # 🔥 FIX 1 y 2: Autoselección y Memoria de BD
        indice_defecto = 0
        ...
        # 🔥 FIX 3: Expander forzado a mantenerse abierto si se está subiendo/creando una BD
...
def muestra_creacion_bdd():
    # 🔥 Le damos un KEY al file_uploader para poder controlar el acordeón
    ...
    if archivoSubido is not None:
        ...
        # Le damos un toque azul para que destaque más
        if st.button("Crear Base de Datos", type="primary"):
            if nombreBd:
                # Modificamos el mensaje para que el usuario entienda qué está pasando
                with st.spinner("Construyendo la base de datos, calculando grupos y asignando espacios. Esto puede tardar unos segundos..."):
                    ...
                    if exito:
                        ...
                        st.session_state['bdActual'] = resultado # La autoseleccionamos de golpe
                        guardar_bd_activa(resultado)
                        import time
                        time.sleep(2.0) # Esperamos 2 segundos completos para que el usuario pueda leer el mensaje de éxito sin estrés
                        st.rerun()
                    else:
