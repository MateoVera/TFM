...
def calcula_grupos_df(conexion, cupos):
    ...
    for _, fila in dfAgrupado.iterrows():
        ...
        particiones = particiona_agrupacion(dictGrados, mGruposTeorico, fila['Curso'], cupos)
...
# 🔥 MODO DIOS: FUNCIÓN PARA RECALCULAR AUTOMÁTICAMENTE EN BACKGROUND 🔥
def recalcula_distribucion_total(conexion, periodo, cuposActivos=None):
    """
    Función 'Modo Dios' para recalcular y distribuir automáticamente
    una vez ingresados los datos en la BDD desde el Excel.
    """
    ...
    # 1. Calcula los grupos teóricos y necesidades
    dfCalculado = calcula_grupos_df(conexion, cuposActivos)
    if dfCalculado.empty:
        return
        
    # 2. Actualiza la base de datos con los cálculos
    actualiza_calculo_grupos(conexion, dfCalculado)
    
    # 3. Toma las autorizaciones oficiales que se acaban de inyectar y ejecuta la distribución física
    dfOriginal = pd.read_sql_query("SELECT * FROM Calculo_Grupos WHERE Cod_Periodo = ?", conexion, params=(periodo,))
    if not dfOriginal.empty:
        # Al pasar {} evitamos editar autorizados, solo forzamos el cálculo de la distribución
...
def genera_excel_oficial(conexion, dfActual, dfCalculado, filtroPeriodo, filtroAsignatura, filtroAgrupacion, filtroIdioma, cuposActivos):
    ...
    hojaActiva.auto_filter.ref = f"A2:AL3000"
        ...
        if not dfActual.empty:
            ...
            with tabComparativas:
                ...
                if tipoComparativa == "📊 Calculados vs Autorizados (Período Actual)":
                    ...
                    if fPeriodo == "Todos":
                        st.warning("⚠️ Selecciona un **Período** específico en los Filtros Globales (ej. 2025/26.2) para identificar el año académico a comparar.")
                    else:
            ...
            with tabDistribucion:
                ...
                if dfFiltrado.empty:
                    ...
                    st.warning("⚠️ Faltan grupos autorizados en la selección filtrada. Por favor, asigna los grupos autorizados en la primera pestaña para continuar.")
                else:
                                ...
                                if fila_check['Tipo_Clase'] != 'M' and pd.notna(fila_check['Grupos_Autorizados']) and fila_check['Grupos_Autorizados'] > 0:
                                    ...
                                    if not m_row.empty and pd.notna(m_row.iloc[0]['Grupos_Autorizados']):
                                        ...
                                        if int(fila_check['Grupos_Autorizados']) < m_aut:
                                            alertas_fusion.append(f"⚠️ **Asignatura {fila_check['Cod_Asignatura']} ({fila_check['Cod_Idioma']})**: Hay menos grupos de **{fila_check['Tipo_Clase']}** ({int(fila_check['Grupos_Autorizados'])}) que de Magistral ({m_aut}). El sistema ha mezclado a alumnos de distintas magistrales para asegurar que nadie se quede sin clase.")
            ...
            with tabAzul:
                ...
                if fPeriodo == "Todos":
                    st.warning("⚠️ Operación bloqueada: Para evitar cruces con datos históricos, debes seleccionar un **Período** específico en los Filtros Globales antes de generar la Tabla Azul.")
